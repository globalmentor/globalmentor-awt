...
/**
 * Grid bag layout class that has additional convenience functionality, including box layout and border layout functionality.
 * <p>
 * This layout can be used in place of {@link BorderLayout} either by using the {@link BorderLayout} constraints or, preferably, by using one of the
 * <code>createBorderConstraints()</code> in this class. Border layout functionality is enhanced by allowing multiple components in each border, the total
 * number in both directions together limited by {@link GridBagLayout#MAXGRIDSIZE}.
 * </p>
 * <p>
 * A component with a basic grid bag layout can be used in place of a horizontal or vertical {@link javax.swing.Box}, with the added benefit that weights can be
 * assigned to each component, using <code>createNextBoxConstraints()</code> for layout constraints when adding components. For example, a vertical layout might
 * add a {@link javax.swing.Box#createGlue()} at the end using constraints of <code>createNextBoxConstraints(BasicGridBagLayout.X_AXIS, 1.0)</code>.
 * </p>
...
public class BasicGridBagLayout extends GridBagLayout {
	...
	/**
	 * Determines the largest x or y coordinate of all components that were added using a <code>GridBagConstraint</code>.
	 * @param axis The axis on which to determine the maximum coordinate, either <code>X_AXIS</code> or <code>Y_AXIS</code>
	 * @return The largest coordinate on the given axis, or <code>-1</code> if no components were added using a <code>GridBagConstraint</code>.
	 ...
	 * @return A rectangle representing the upper left x and y coordinates as well as the extent of the grid.
	 */
	...
	/**
	 * Creates constraints appropriate for laying out components in a row in a single column or row on the horizontal or vertial axis.
	 * <p>
	 * The constraints will have a weight of <code>0.0</code>.
	 * </p>
	 * @param axis The axis along which components are being laid out, either <code>X_AXIS</code> or <code>Y_AXIS</code>
	 * @return A grid bag constraint object for adding a new component in single file along the horizontal or vertical axis.
	 * @throws IllegalArgumentException Thrown if the axis value is unrecognized
	 * @see #X_AXIS
	 * @see #Y_AXIS
	 */
	 ...
	 * Creates constraints appropriate for laying out components in a row in a single column or row on the horizontal or vertial axis.
	 * <p>
	 * This version uses a default anchor of <code>GridBagConstraints.CENTER</code> and a default fill of <code>GridBagConstraints.BOTH</code>. No insets are
	 * used.
	 * </p>
	 * @param axis The axis along which components are being laid out, either <code>X_AXIS</code> or <code>Y_AXIS</code>
	 * @param weight An amount specifying how to distribute the extra space along the axis.
	 * @return A grid bag constraint object for adding a new component in single file along the horizontal or vertical axis.
	 * @throws IllegalArgumentException Thrown if the axis value is unrecognized
	 * @see #X_AXIS
	 * @see #Y_AXIS
	 ...
	 * Creates constraints appropriate for laying out components in a row in a single column or row on the horizontal or vertial axis.
	 * <p>
	 * This version uses a default weight of <code>0.0</code>.
	 * </p>
	 * @param axis The axis along which components are being laid out, either <code>X_AXIS</code> or <code>Y_AXIS</code>
	 ...
	 * @param position The position (center or border) in which to place the component; one of the <code>BorderLayout</code> position constants.
	 * @param displacement The zero-based displacement from the center, allowing multiple borders away from the center component.
	 * @return A grid bag constraint object for adding a new component in the center or border of this panel.
	 * @see BorderLayout
	 */
	//TODO fix	public static GridBagConstraints createBorderConstraints(final String position, final int displacement) 

	/**
	 * Creates constraints appropriate for placing a component in a the center or in a border of the panel. This mimics the functionality of
	...
	*/
	 ...
	 * <p>
	 * A constraint of <code>BorderLayout.CENTER</code> always returns 0.
	 * </p>
	 * @param border The border in which to place the component, one of the <code>BorderLayout</code> constants such as <code>Border.NORTH</code>.
	 * @return The next available zero-based index suitable for placing a component in the given border, or -1 if no next index is available.
	 * @throws IllegalArgumentException Thrown if constraints is not a valid <code>BorderLayout</code> constraint.
	 */
	...
	/**
	 * Creates constraints appropriate for laying out a component in one of the four borders or the center. This method considers the center to be cell (
	 * <code>MAXGRIDSIZE</code>/2, <code>MAXGRIDSIZE</code>/2). Components in the borders are placed at their farthest position <em>away</em> from the center, so
	 * that higher indexes move towards the center. This method uses the first free index available in the given border.
	 * <p>
	 * Components in north and south borders will fill multiple cells, so that components in west and east borders will always be between components in the north
	 * and south borders.
	 * </p>
	 * @param border The border in which to place the component, one of the <code>BorderLayout</code> constants such as <code>Border.NORTH</code>.
	 * @return A grid bag constraint object for adding a new component in one of the borders.
	 * @throws IllegalArgumentException Thrown if constraints is not a valid <code>BorderLayout</code> constraint.
	...
	/**
	 * Creates constraints appropriate for laying out a component in one of the four borders or the center. The center is considered to be cell (
	 * <code>MAXGRIDSIZE</code>/2, <code>MAXGRIDSIZE</code>/2). Components in the borders are placed at their farthest position <em>away</em> from the center, so
	 * that higher indexes move towards the center.
	 * <p>
	 * Components in north and south borders will fill multiple cells, so that components in west and east borders will always be between components in the north
	 * and south borders.
	 * </p>
	 * @param border The border in which to place the component, one of the <code>BorderLayout</code> constants such as <code>Border.NORTH</code>.
	 * @param index The zero-based index toward from the center; the true index taking into account positive and negative directions will be correctly calculated.
	 *          Every non-negative index value is ignored for a border of <code>BorderLayout.CENTER</code>.
	 * @return A grid bag constraint object for adding a new component in one of the borders.
	 * @throws IllegalArgumentException Thrown if constraints is not a valid <code>BorderLayout</code> constraint or the given index is negative.
	 */
	public GridBagConstraints createBorderConstraints(String border, final int index) throws IllegalArgumentException {
		...
		if(BorderLayout.NORTH.equals(border)) { //north
			...
			return new GridBagConstraints(0, MAXGRIDSIZE - 1 - index, MAXGRIDSIZE, 1, 1.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, NO_INSETS,
					0, 0);
		} else if(BorderLayout.WEST.equals(border)) { //west
	...
	/**
	 * Converts the given border constrataint to one of the absolute canonical border constraints.
	 * <p>
	 * Internationalization border constraints such as <code>BorderLayout.LINE_START</code> will be converted to absolute border constraints such as
	 * <code>BorderLayout.WEST</code> assuming left-to-right top-to-bottom component orientation. (All absolute values become relative internationalized values
	 * when laid out using this class, however.)
	 * </p>
	 * @param border The border in which to place the component, one of the <code>BorderLayout</code> constants such as <code>Border.PAGE_START</code> or
	 *          <code>Border.NORTH</code>.
	 * @return A canonical absolute border constraint, one of <code>Border.CENTER</code>, <code>Border.NORTH</code>, <code>Border.EAST</code>,
	 *         <code>Border.SOUTH</code>, and <code>Border.WEST</code>.
	 * @throws IllegalArgumentException Thrown if constraints is not a valid <code>BorderLayout</code> constraint.
	 */
	...
	public void addLayoutComponent(Component comp, Object constraints) {
		if(constraints instanceof String) //if this seems to be a border constraint
			...
			//if this is not a string, it's probably a grid bag constraint
