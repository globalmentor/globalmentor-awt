...
public class Colors {
	...
	/**
	 * Compares one color with another, based on their alpha, red, green, and blue values, taking into account that one or both colors may be <code>null</code>.
	 * If one color is <code>null</code> but the other is not, the <code>null</code> color is considered to be less than the non-<code>null</code> color
	...
	public static final int compareTo(final Color color1, final Color color2) {
		if(color1 != null && color2 != null) //if both colors are non-null
			...
			//if one color is null and the other isn't
