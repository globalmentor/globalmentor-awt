...
public class Containers {
	 ...
	 * implement <code>Modifiable</code> will recursively be searched for children that do implement <code>Modifiable</code>.
	 * <p>
	 * This method is useful for a container that has had its content saved and wishes to express that all its content, including that of its children, are now
	 * unmodified. Note that this method is not useful for the converse situation of setting modified to <code>true</code>&mdash;just because one component has
	...
	/**
	 * Verifies each child component that implements <code>Verifiable</code>. If any child does not implement <code>Verifiable</code>, its children will be
	 * searched for components that implements <code>Verifiable</code>.
	 * @param container The container the children of which to verify.
	 * @return <code>true</code> if all the child component contents are valid&mdash;if no descendant that implements <code>Verifiable</code> returns
	 *         <code>false</code>.
