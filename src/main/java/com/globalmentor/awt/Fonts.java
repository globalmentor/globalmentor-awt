 ...
 * <p>
 * When needed, this class also creates an array representing an enumeration of family names of all fonts stored on the system. This array, once created, will
 * never be released from memory.
 * </p>
...
public class Fonts {
	...
	/**
	 * Returns a sorted array of names of available fonts. The returned array is shared by other objects in the system, and should not be modified. The first call
	 * to this method will create and cache the array.
	...
	/**
	 * Gets a new font. This returns a <code>Font</code> from a cache if a cached font exists and its memory has not been reclaimed. The the font has not been
	 ...
	 * <p>
	 * Once a font is found for a particular character, that font family name is stored with the character's Unicode block so that retrieving a font for the same
	 * block character will be much faster in the future. The name will also be stored keyed to the character itself. If a character is requested for the same
	 * Unicode block in the future but that specific character is not available, the font family name will be searched with the character as the key. This speeds
	 * searches yet saves space based on the assumption that most fonts, if they contain one character in a Unicode block, will contain all characters in a
	 * Unicode block.
	 * </p>
	 * <p>
	 * If no fonts on the system support the given character, this method will flag that character as unsupported so that during the next call to this method all
	 * the fonts will not be searched.
	 * </p>
	 * <p>
	 * This method also keeps a list of recommended fonts for several character blocks so that those fonts may be searched first, if the character is not
	...
	/**
	 * A key for storing fonts in the font table. Modified from <code>javax.swing.text.StyleContext.FontKey</code> version, 1.65 02/02/00 by Timothy Prinzing,
	 * Copyright 1997-2000 Sun Microsystems, Inc., because that version is private and inaccessible.
	 */
	protected static class FontKey {
		...
		/**
		 * Compares this font key to the specifed object. The result is <code>true</code> if and only if the argument is not <code>null</code> and is a
		 * <code>FontKey</code> object with the same name, style, and point size as this font key.
